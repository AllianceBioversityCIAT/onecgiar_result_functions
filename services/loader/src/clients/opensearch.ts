...
export class OpenSearchClient {
  ...
  constructor(endpoint?: string, indexPrefix = 'prms-results') {
    this.endpoint = endpoint || (process as any).env.OPENSEARCH_ENDPOINT || 'https://localhost:9200';
    this.indexPrefix = indexPrefix;
    
    if ((process as any).env.OPENSEARCH_USERNAME && (process as any).env.OPENSEARCH_PASSWORD) {
      this.auth = {
  ...
  private async makeRequest(method: string, path: string, body?: any): Promise<any> {
    ...
    try {
      const response = await fetch(url, {
        ...
        ...(process.env.NODE_ENV !== 'production' && { rejectUnauthorized: false } as any)
      });

      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`OpenSearch ${method} ${path} failed: ${response.status} ${response.statusText} - ${errorText}`);
      }

      return await response.json();
    } catch (error) {
      console.error(`[OpenSearchClient] Request failed: ${method} ${url}`, error);
      throw error;
    }
  }

  async indexResult(result: ProcessedResult): Promise<any> {
    ...
    try {
      ...
      const response = await this.makeRequest(
        'PUT',
        `/${indexName}/_doc/${documentId}?refresh=true`,
        document
      );

      console.log(`[OpenSearchClient] Successfully indexed result ${documentId}`, {
        index: indexName,
      ...
      console.error(`[OpenSearchClient] Error indexing result ${documentId}:`, error);
      throw error;
    }
  }
  ...
  async bulkIndex(results: ProcessedResult[]): Promise<any> {
    ...
    try {
      ...
      if (errors.length > 0) {
        console.warn(`[OpenSearchClient] Bulk index completed with ${errors.length} errors:`, errors);
      } else {
        console.log(`[OpenSearchClient] Bulk index completed successfully for ${results.length} results`);
      }
  ...
  async ensureIndex(resultType: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[OpenSearchClient] Error ensuring index ${indexName}:`, error);
