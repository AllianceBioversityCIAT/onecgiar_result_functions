...
export class OtherOutputProcessor implements ProcessorInterface {
  ...
  async process(result: ResultData): Promise<ProcessingResult> {
    ...
    try {
      this.logger.info("Starting other output processing", resultId, {
        type: result.type,
      });
      ...
      this.logger.error("Other output processing failed", resultId, error);
  ...
  /**
   * Process multiple other output results in parallel (with concurrency limit)
   */
  async processBatch(
    results: ResultData[],
    concurrency = 5
