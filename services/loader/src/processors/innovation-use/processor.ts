...
export class InnovationUseProcessor implements ProcessorInterface {
  ...
  async process(result: ResultData): Promise<ProcessingResult> {
    ...
    try {
      this.logger.info("Starting innovation use processing", resultId, {
        type: result.type,
      });
      ...
      this.logger.error("Innovation use processing failed", resultId, error);
  ...
  /**
   * Process multiple innovation use results in parallel (with concurrency limit)
   */
  async processBatch(
    results: ResultData[],
    concurrency = 5
