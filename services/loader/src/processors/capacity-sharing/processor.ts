...
export class CapacitySharingProcessor implements ProcessorInterface {
  ...
  async process(result: ResultData): Promise<ProcessingResult> {
    ...
    try {
      this.logger.info("Starting capacity sharing processing", resultId, {
        type: result.type,
      });
      ...
      this.logger.success("Capacity sharing processed successfully", resultId, {
  ...
  /**
   * Process multiple capacity sharing results in parallel (with concurrency limit)
   */
  async processBatch(
    results: ResultData[],
    concurrency = 5
