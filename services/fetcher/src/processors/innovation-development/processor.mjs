...
export class InnovationDevelopmentProcessor  {
  logger;
  ...
  async process(result) {
    ...
    try {
      this.logger.info("Starting innovation development processing", resultId, {
        type: result.type,
      });
      ...
      this.logger.success(
        "Innovation development processed successfully",
        resultId,
        {
  ...
  /**
   * Process multiple innovation development results in parallel (with concurrency limit)
   */
  async processBatch(
    results,
    concurrency = 5
