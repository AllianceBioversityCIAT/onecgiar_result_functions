...
export class OpenSearchClient {
  ...
  getIndexName(resultType) {
    const slugifiedType = typeof resultType === "string"
      ? resultType.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/(^_|_$)+/g, "")
      : resultType;
    return `${this.indexPrefix}-${slugifiedType}`;
  }
  ...
  async bulkIndex(results) {
    ...
    try {
      ...
      if (errors.length > 0) {
        ...
        console.log(
          `[OpenSearchClient] Bulk index completed successfully for ${results.length
